...
课程项目样本量可能较小，结果仅用于探索性分析，不应作因果结论的过度外推。房源来自用户提供的房天下上海租赁挂牌 CSV 快照，并非交易成交数据；挂牌真实性、时效性、授权范围和平台条款需由项目组在公开发布前进一步确认。当前数据不含通勤、装修、社区、安全、电梯、押金和中介费，系统不得把缺失值描述成真实观测。未来正式研究需要伦理审批、预注册、功效分析、更大样本、数据保留期限和退出/删除机制。不得伪造实验结果。
